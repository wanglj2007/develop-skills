---
name: csharp-api
description: 在任意基于 C# 的后端项目中新增、重构或审查业务管理 API 时使用。适用于 ASP.NET Core 的 Controller、Service 与 Models 分层设计和修改，尤其是涉及列表、保存、新增、编辑、启用、禁用、删除或批量删除接口时；统一接口路由、HTTP 方法、模型目录和服务分层，并以项目现有约定为准。
---
